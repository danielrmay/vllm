...
def _assert_greedy_equivalent(baseline, cached, tie_margin_nats=0.05):
    ...
    for seq_idx, ((base_tokens, base_lps), (cached_tokens, cached_lps)) in enumerate(
        ...
        total_positions += len(base_tokens)
    ...
    # Power floor: near-tie tolerance must not silently erode the claim. A
    # systematic small-magnitude corruption looks like MANY early near-tie
    # stops; genuine tie-break noise is rare. Fail loudly rather than pass
    # having verified almost nothing.
    assert early_stopped <= 2 and matched_positions >= 0.8 * total_positions, (
        f"near-tie early stops eroded the equivalence claim: only "
        f"{matched_positions}/{total_positions} positions matched exactly, "
        f"{early_stopped} sequences stopped on tolerated ties"
    )
