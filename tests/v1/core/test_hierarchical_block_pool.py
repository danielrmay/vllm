...
def test_large_alloc_free_roundtrip():
    ...
    pool.free_blocks(larges, large_block=True)
    assert pool.get_num_free_large_blocks() == NUM_LARGE - 1
    for blk in larges:
        meta = pool.large_block_metas[blk.block_id]
...
def test_repurpose_as_small_evicts_stale_large_hash():
    ...
    pool.free_blocks([large], large_block=True)
...
def test_free_mixed_list_identity_classification():
    """free_blocks must classify each block by IDENTITY, not by the caller's
    flag: call sites can free mixed lists whose numeric ids collide (a large
    ...
    pool.free_blocks([blk for blk in larges if blk is not large], large_block=True)
    small = None
    ...
    # Free the mixed list in one call with the (untrustworthy) flag set.
    pool.free_blocks([small, large], large_block=True)
