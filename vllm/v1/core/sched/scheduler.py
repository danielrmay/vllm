...
def _configured_connector_names(
    ...
    # A falsy/absent connector name is not a configured connector; keeping
    # "" would spuriously trip connector-gated refusals.
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Exact-name allowlist, fail-closed: a substring match would let an
        # unknown "NixlSomethingConnector" that does NOT ship full mamba
        # state enable the shortcut and silently poison downstream caches.
        self._connector_transfers_full_mamba_state = bool(loadable) and all(
            name == "NixlConnector" for name in loadable
        )
