...
class BlockPool:
    ...
    def free_blocks(
        ...
        assert self.free_large_block_queue is not None
        # Classify each block by IDENTITY rather than
        # trusting the ``large_block`` flag. Callers can free a
        ...
        # only safe discriminator. The ``large_block`` flag is kept as an API
        # hint for callers but is not load-bearing.
        del large_block
        N = self.large_block_factor
    ...
    def evict_blocks(self, block_ids: set[int]) -> None:
        ...
        """
        assert self.large_block_factor == 1 or not block_ids, (
